      ...
      {		
         ...
         if(bomber!=null)
         {
            ...
            if(dist <= (SIZE*2))
            {
               ...
               channels[0].noteOn((int)(Math.random()*11)+30, (int)(Math.random()*31)+60);
               return true;
            }
         }
         return false;
    ...
    //post: returns true if the Shell curr collides with a player (tank) - destroyes tanks and updates score
      private static boolean checkPlayerCollision(Shell curr)
      {
         ...
         for(int i=0; i<players.length; i++)
         {
            ...
            if(curr.getOwner() == i || cX < 0 || curr.getType().equals("bomb"))	//don't shoot ourselves, don't let bombs destroy tanks (unfair)
               continue;
            ...
            if(dist <= (SIZE) && !target.onFire())
            {
               ...
               //sound elements
               channels[0].programChange(instr[TAIKO].getPatch().getProgram());
               channels[0].noteOn((int)(Math.random()*11)+30, 100);
               return true;
            }
         }
         return false;
